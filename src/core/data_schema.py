...
class CallRecord(BaseModel):
    ...
    product_name: Optional[str] = Field(None, description="Product discussed")
    quantity: Optional[int] = Field(None, description="Quantity if applicable")
    amount: Optional[float] = Field(None, description="Transaction amount")
    order_id: Optional[str] = Field(None, description="Order identifier")
    
    # Derived labels from analysis
    connection_status: Optional[CallConnectionStatus] = None
...
class CallDataFrame:
    ...
    def filter_by_date_range(self, start_date: datetime, end_date: datetime) -> pd.DataFrame:
        ...
        """
        mask = (
            (self.df['start_time'] >= start_date) & 
            (self.df['start_time'] <= end_date)
        )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        connected_pct = (
            connection_counts.get('Connected', 0) / total_calls * 100 
