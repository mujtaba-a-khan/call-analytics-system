...
class QAInterface:
    ...
    def _render_response_data(self, data: Any) -> None:
        ...
        if isinstance(data, pd.DataFrame):
            ...
            if "metrics" in data:
                # Render metrics
                metrics = [
                    MetricValue(value=v, label=k.replace("_", " ").title())
    ...
    def _render_context_panel(self) -> None:
        ...
        with st.expander("⚙️ Settings"):
            st.checkbox("Show data sources", value=True, key="qa_show_sources")

            st.checkbox("Include visualizations", value=True, key="qa_include_viz")

            st.slider(
                "Response detail level", min_value=1, max_value=5, value=3, key="qa_detail_level"
            )
    ...
    def _generate_response(self, query: str, intent: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        response = {"role": "assistant", "timestamp": datetime.now(), "sources": []}
    ...
    def _handle_search_query(
        ...
        results = self.search_engine.search(query=search_terms, top_k=5, threshold=0.35)
    ...
    def _handle_comparison_query(
        ...
        if compare_what == "periods":
            ...
            if "agent_id" in data.columns:
                agent_comparison = (
                    data.groupby("agent_id")
                    .agg({"call_id": "count", "outcome": lambda x: (x == "connected").mean() * 100})
                    .round(2)
                )
    ...
    def _generate_llm_response(self, query: str) -> str:
        ...
        try:
            ...
            result = self.llm_client.generate(prompt=query, system_prompt=system_prompt)
    ...
    def _generate_suggestions(self) -> list[str]:
        ...
        suggestions.extend(["What is the total number of calls?", "Show me today's call summary"])
...
def render_qa_interface(
    storage_manager: StorageManager, vector_store=None, llm_client=None
) -> None:
    """
